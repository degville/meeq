...
/// should look like:
/// chuck meeq.ck:16:8:8080:8000:0:120:71:41

if( me.args() ){

	Std.atoi(me.arg(0)) => maxx; 						/// width
	Std.atoi(me.arg(1)) => maxy; 						/// height
	Std.atoi(me.arg(2)) => hostport; 					/// OSC host port
	Std.atoi(me.arg(3)) => receiveport;					/// OSC receiving port
...
fun void columnSet(int x,int y){
    ...
    xmit.startMsg(prefix+"/led_col", "ii");
    x => xmit.addInt;
    y => xmit.addInt;
}
